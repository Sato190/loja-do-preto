(()=>{
  ...
  function prepare(root=document){
    root.querySelectorAll?.('.brand-shield').forEach(media=>{media.classList.add('history-media');const image=media.querySelector('img');if(image&&/logo-clean/i.test(image.src)){image.src='/assets/hero-showroom.png';image.alt='Ambiente da Loja do Preto'}});
  ...
  const start=()=>{document.addEventListener('error',event=>{const img=event.target;if(!(img instanceof HTMLImageElement)||img.dataset.fallbackApplied)return;img.dataset.fallbackApplied='1';img.src=img.classList.contains('portal-logo')||img.closest('.brand,.logo,.portal-brand')?'/assets/logo-clean.png':'/assets/hero-showroom.png'},true);signatures();prepare();mutations.observe(document.body,{childList:true,subtree:true});gallery();setTimeout(()=>{prepare();updateGalleryProgress();animateVehicleNumbers()},500);parallax()};
