...
create unique index if not exists admin_profiles_login_unique
on public.admin_profiles(lower(login))
...
on public.admin_login_attempts(lower(login),ip_hash,created_at desc);

create or replace function public.can_access_admin()
returns boolean language sql stable security definer set search_path=public as $$
  select exists(
    select 1 from public.admin_profiles
    where user_id=auth.uid() and active=true and status='active' and removed_at is null
  )
$$;
...
-- Rollback strategy: retain account data; remove only the index/function after
-- confirming no deployed authentication flow depends on them.
