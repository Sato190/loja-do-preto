(()=>{if(!document.querySelector('link[href$="motion.css"]')){const l=document.createElement('link');l.rel='stylesheet';l.href='/motion.css';document.head.append(l)}if(!document.querySelector('script[src$="navigation.js"]')){const s=document.createElement('script');s.src='/navigation.js';document.head.append(s)}if(!document.querySelector('script[src$="motion.js"]')){const s=document.createElement('script');s.src='/motion.js';document.head.append(s)}
  ...
  const meta=(selector,attributes)=>{let element=document.head.querySelector(selector);if(!element){element=document.createElement(attributes.tag||'meta');document.head.append(element)}Object.entries(attributes).filter(([key])=>key!=='tag').forEach(([key,value])=>element.setAttribute(key,value));return element};const canonical=`https://loja-do-preto.vercel.app${location.pathname}`;meta('link[rel="canonical"]',{tag:'link',rel:'canonical',href:canonical});meta('meta[property="og:title"]',{property:'og:title',content:document.title});meta('meta[property="og:description"]',{property:'og:description',content:document.querySelector('meta[name="description"]')?.content||''});meta('meta[property="og:type"]',{property:'og:type',content:'website'});meta('meta[property="og:url"]',{property:'og:url',content:canonical});
  ...
  input?.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(async()=>{
    ...
    if(!db)return;const reference=q.match(/^(?:ref\s*)?(\d+)$/i);let search=db.from('vehicles').select('id,slug,reference,brand,model,version,year,price,body_type,vehicle_type').eq('active',true);if(reference)search=search.eq('reference',Number(reference[1]));else{const term=`%${q}%`;search=search.or(`brand.ilike.${term},model.ilike.${term},version.ilike.${term},category.ilike.${term},body_type.ilike.${term},transmission.ilike.${term},fuel.ilike.${term}`)}const {data}=await search.limit(8);
    out.innerHTML=(data||[]).length?data.map(v=>`<a class="search-result" href="/veiculos/${encodeURIComponent(v.slug||v.id)}"><span><b>${safe(v.brand)} ${safe(v.model)}</b><small>${v.reference?`REF ${String(v.reference).padStart(3,'0')} · `:''}${safe(v.version||v.year)}</small></span><strong>${money(v.price)}</strong></a>`).join(''):'<p>Nenhum veículo encontrado.</p>';
  },250)});
  document.querySelector('#sell-form')?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.currentTarget),lead=Object.fromEntries(f),status=e.currentTarget.querySelector('.form-status');status.textContent='Enviando solicitação…';const{error}=db?await db.rpc('submit_public_lead',{lead_source:'sell_page',lead_name:String(lead.name||'').trim(),lead_phone:lead.phone||null,lead_email:null,lead_vehicle:lead.vehicle||null,lead_details:lead,website:lead.website||null}):{error:true};if(error){status.textContent='Não foi possível salvar agora. Tente novamente.';return}const msg=`Olá! Quero vender ou trocar meu carro.\nNome: ${f.get('name')}\nVeículo: ${f.get('vehicle')}\nAno: ${f.get('year')}\nQuilometragem: ${f.get('mileage')}\nTelefone: ${f.get('phone')}`;open(wa(settings.whatsapp,msg),'_blank','noopener');status.textContent='Solicitação salva. Continue pelo WhatsApp.'});
