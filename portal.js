(()=>{if(!document.querySelector('script[src$="navigation.js"]')){const s=document.createElement('script');s.src='/navigation.js';document.head.append(s)}
  ...
  const wa=(number,message)=>`https://wa.me/${String(number||'').replace(/\D/g,'')}?text=${encodeURIComponent(message)}`;
  ...
  input?.addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(async()=>{
    ...
    if(!db)return; const term=`%${q}%`;const {data}=await db.from('vehicles').select('id,slug,brand,model,version,year,price').eq('active',true).or(`brand.ilike.${term},model.ilike.${term},version.ilike.${term},category.ilike.${term}`).limit(8);
    out.innerHTML=(data||[]).length?data.map(v=>`<a class="search-result" href="/estoque/${safe(v.slug||v.id)}"><span><b>${safe(v.brand)} ${safe(v.model)}</b><small>${safe(v.version||v.year)}</small></span><strong>${money(v.price)}</strong></a>`).join(''):'<p>Nenhum veículo encontrado.</p>';
  },250)});
  document.querySelector('#sell-form')?.addEventListener('submit',e=>{e.preventDefault();const f=new FormData(e.currentTarget);const msg=`Olá! Quero vender ou trocar meu carro.\nNome: ${f.get('name')}\nVeículo: ${f.get('vehicle')}\nAno: ${f.get('year')}\nQuilometragem: ${f.get('mileage')}\nTelefone: ${f.get('phone')}`;open(wa(settings.whatsapp,msg),'_blank');e.currentTarget.querySelector('.form-status').textContent='Informações preparadas. Continue pelo WhatsApp.'});
